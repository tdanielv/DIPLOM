        ...
        if self.use_pk_only_optimization() and self.source_attrs:
            ...
            try:
                ...
                if is_simple_callable(value):
                    # Handle edge case where the queryset `source` argument
                    # points to a `get_relationship()` method on the model.
                    value = value()

                # Handle edge case where queryset `source` argument points
...
class HyperlinkedRelatedField(RelatedField):
    ...
    def to_representation(self, value):
        ...
        try:
            ...
            msg = (
                'Could not resolve URL for hyperlinked queryset using '
...
class HyperlinkedIdentityField(HyperlinkedRelatedField):
    ...
    def use_pk_only_optimization(self):
        ...
        # to run the 'only get the pk for this queryset' code.
        return False


class SlugRelatedField(RelatedField):
    """
    A read-write field that represents the target of the queryset
...
class ManyRelatedField(Field):
    ...
    a ManyRelatedField with a child queryset.

    The `ManyRelatedField` class is responsible for handling iterating through
    the values and passing each one to the child queryset.

    This class is treated as private API.
    You shouldn't generally need to be using this class directly yourself,
    and should instead simply set 'many=True' on the queryset.
    """
    initial = []
    default_empty_html = []
    default_error_messages = {
